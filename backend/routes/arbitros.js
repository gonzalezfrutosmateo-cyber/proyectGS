...
router.get('/', envolver(async function (req, res) {
    const [rows] = await pool.query(
        'SELECT id_arbitro AS id, nombre, apellido FROM arbitro WHERE activo = 1 ORDER BY apellido'
    );
    res.json(rows);
}));
...
router.put('/:id', requireRole('admin'), envolver(async function (req, res) {
    ...
    await pool.query(
        'UPDATE arbitro SET nombre = ?, apellido = ? WHERE id_arbitro = ?',
...
router.delete('/:id', requireRole('admin'), envolver(async function (req, res) {
    await pool.query('UPDATE arbitro SET activo = 0 WHERE id_arbitro = ?', [req.params.id]);
    res.json({ ok: true });
}));
